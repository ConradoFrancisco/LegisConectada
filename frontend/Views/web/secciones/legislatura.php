...
//echo '</pre>';
//if (empty($legislatura->url_normativas)) {
//	echo 'vacia';
//}else{
//	echo 'tiene';
//}
		...
		<div class="hvrbox-layer_top">
			<div class="container">
				<div class="overlay-text text-center">
					...
					<h4 class="region "><?= $legislatura->nom_provicia . ', ' . $legislatura->nombre_pais ?></h4>
				</div>
			</div>
		</div>
	</div>
							...
							<div class="project-quick-info">
								<div class="media-body ">
									...
									<ul class="top-nav-social">
										...
										<?php endif; ?>
										<?php if ($legislatura->url != '') : ?>
											<li><a href="<?= $legislatura->url ?>" class="youtube" target="_blank"> <i class="fa fa-globe"></i> </a>
											</li>
										<?php endif; ?>
									</ul>
...
-->
			...
			<section id="autoridades">
				<?php if ($autoridades) : ?>
					...
					<div class="row justify-content-center">
						<?php foreach ($autoridades as $data) : ?>
							<div class="text-center col-md-3 alert alert-light" role="alert">
								<?= '<strong>' . $data->cargo . '</strong><br> ' . $data->apellido . ', ' . $data->nombre ?>
							</div>
						<?php endforeach; ?>
					</div>
				<?php endif; ?>
			...
			<section class="normativas">
				...
				<?php
					...
					if (empty($legislatura->url_normativas)) {
						...
						echo '<hr><a target="_blank" href="' . $legislatura->url_normativas . '">Normativas Publicadas por ' . $legislatura->nombre . '</a><hr>';
					}
				//			echo '<hr>Sin información por el momento<hr>';
				endif;
				?>
			...
			?>
				...
				<div class="owl-carousel" id="noticias">
					<?php foreach ($publicaciones as $publicacion) : ?>
						<div class="single-bolg hover01 ">
							<figure style="    background: url(<?php echo base_url(); ?>/static/web/images/slider/banner_paginas.png) center;  height: 26.652452025586353vh;  display: flex;  align-items: center;
  justify-content: center;"><img src="<?= base_url($publicacion->foto) ?>" class="img-responsive ig-thumbnail" tyle=" position: relative;margin: auto;top: 0;left: 0;right: 0;bottom: 0;">
							...
							<div class="blog-content">
								<?php
								$segments = array('Prensa', convert_accented_characters(url_title($publicacion->titulo), 'underscore', TRUE), $publicacion->id);
								?>
